      ...
      >
         <template #cell(actions)="{ item }">
            <div class="text-center" style="text-wrap: nowrap">
               <!-- edit -->
               <b-link
                  v-if="$can('OrderToSendBusinessTripEdit', 'permissions')"
                  :to="{
                     name: 'EditOrderToSendBusinessTrip',
                     params: { id: item.id }
                  }"
               ...
               <!-- view -->
               <b-link
                  :to="{ name: 'ViewOrderToSendBusinessTrip', params: { id: item.id } }"
                  v-b-tooltip.hover.top="$t('View')"
                  v-if="
                     $can('OrderToSendBusinessTripView', 'permissions') ||
                     $can('OrderToSendBusinessTripSignerView', 'permissions')
                  "
                  class="mr-1 cursor-pointer"
               >
               ...
               <!-- delete -->
               <b-link
                  v-if="$can('OrderToSendBusinessTripDelete', 'permissions') && item.canDelete"
               ...
               <!-- Sign -->
               <b-link
                  v-if="$can('OrderToSendBusinessTripSign', 'permissions') && item.canSign"
               ...
               <!-- Cancel -->
               <template v-if="$can('OrderToSendBusinessTripCancel', 'permissions') && item.canCancel">
                  <b-link
               ...
               <!-- document history -->
               <HistoryModalButton :id="item.id" :table-id="item.tableId || 135" />
            </div>
         </template>
         <template #status>
            <b-row>
...
export default {
   ...
   computed() {
      if (JSON.parse(localStorage.getItem('filterData10'))) {
         this.filter = JSON.parse(localStorage.getItem('filterData10'));
      } else {
   ...
   watch: {
      ...
      filter: {
         handler(newValue, oldValue) {
            localStorage.setItem('filterData10', JSON.stringify(newValue));
   ...
   methods: {
      ...
      Refresh() {
         this.isBusy = true;
         if (this.$route.name == 'OrderToSendBusinessTripForSigner') {
            OrderToSendBusinessTripService.GetListForSigner(this.filter)
               .then((res) => {
            ...
            OrderToSendBusinessTripService.GetList(this.filter)
               .then((res) => {
                  this.items = res.data.rows;
                  this.filter.total = res.data.total;
               })
      ...
      Delete(item) {
         this.$swal.fire({
            ...
            preConfirm: () => {
               return OrderToSendBusinessTripService.Delete(item.id)
                  .then(() => {
      ...
      Sign(signedData, isPinfl) {
         ...
         OrderToSendBusinessTripService.Sign({
      ...
      Cancel(item) {
         this.$swal.fire({
            ...
            preConfirm: (msg) => {
               return OrderToSendBusinessTripService.Cancel({
                  id: item.id,
                  message: msg,
                  signedData: 'cancel'
               })
