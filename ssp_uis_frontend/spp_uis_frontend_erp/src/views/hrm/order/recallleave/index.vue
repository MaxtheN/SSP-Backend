      ...
      >
         ...
         <template #cell(actions)="{ item }">
            <div class="text-center" style="text-wrap: nowrap">
               <!-- edit -->
               <b-link
                  v-if="$can('RecallLeaveEdit', 'permissions')"
                  :to="{
                     name: 'EditRecallLeave',
                     params: { id: item.id }
                  }"
               ...
               <!-- view -->
               <b-link
                  :to="{ name: 'ViewRecallLeave', params: { id: item.id } }"
                  v-b-tooltip.hover.top="$t('View')"
                  v-if="$can('RecallLeaveView', 'permissions') || $can('RecallLeaveSignerView', 'permissions')"
                  class="mr-1 cursor-pointer"
               >
               ...
               <!-- delete -->
               <b-link
                  v-if="$can('RecallLeaveDelete', 'permissions') && item.canDelete"
               ...
               <!-- Sign -->
               <b-link
                  v-if="$can('RecallLeaveSign', 'permissions') && item.canSign"
               ...
               <!-- Cancel -->
               <template v-if="$can('RecallLeaveCancel', 'permissions') && item.canCancel">
                  <b-link
               ...
               <!-- document history -->
               <HistoryModalButton :id="item.id" :table-id="item.tableId || 90" />
            </div>
         </template>
      </form-table-hrm>
...
export default {
   ...
   watch: {
      ...
      filter: {
         handler(newValue, oldValue) {
            localStorage.setItem('filterData9', JSON.stringify(newValue));
   ...
   computed() {
      if (JSON.parse(localStorage.getItem('filterData9'))) {
         this.filter = JSON.parse(localStorage.getItem('filterData9'));
      } else {
   ...
   methods: {
      ...
      Refresh() {
         this.isBusy = true;
         if (this.$route.name == 'RecallLeaveForSigner') {
            RecallLeaveService.GetListForSigner(this.filter)
               .then((res) => {
            ...
            RecallLeaveService.GetList(this.filter)
               .then((res) => {
                  this.items = res.data.rows;
                  this.filter.total = res.data.total;
               })
      ...
      Delete(item) {
         this.$swal.fire({
            ...
            preConfirm: () => {
               return RecallLeaveService.Delete(item.id)
                  .then(() => {
      ...
      Cancel(item) {
         this.$swal.fire({
            ...
            preConfirm: (msg) => {
               return RecallLeaveService.Cancel({
                  id: item.id,
                  message: msg,
                  signedData: 'cancel'
               })
