...
export default {
   ...
   data() {
      return {
         ...
         TablesField: [
            ...
            {
               key: 'RecallLeaveId',
               label: this.$t('RecallLeave'),
               sortable: true
            },
            {
               key: 'RecallLeaveTableId',
               label: this.$t('RecallLeaveTable'),
               sortable: true
            },
            {
               key: 'startOn',
   ...
   computed: {
      ...
      IframeSrc() {
         return axios.defaults.baseURL + `hrm/RecallLeave/DownloadPdf?id2=${this.Data?.id2}&lang=${this.getPdfLang()}`;
      }
   },
   methods: {
      Refresh() {
      ...
      DownloadTemplate1() {
         changeDocument.DownloadTemplate('/hrm/RecallLeave/DownloadTemplate', this.$t('employeeleaveorder'));
      },
      UploadFile(event) {
         changeDocument.UploadFile('/hrm/RecallLeave/UploadFile', event).then((res) => {
            this.Data.files.length = 0;
            this.Data.files.push(res.data[0]);
            changeDocument
               .Update('/hrm/RecallLeave/Update', this.Data)
               .then((res) => {
