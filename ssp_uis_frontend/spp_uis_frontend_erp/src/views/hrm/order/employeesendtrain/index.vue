      ...
      >
         <template #cell(actions)="{ item }">
            <div class="text-center" style="text-wrap: nowrap">
               <!-- edit -->
               <b-link
                  v-if="$can('EmployeeSendTrainEdit', 'permissions')"
                  :to="{
                     name: 'EditEmployeeSendTrain',
                     params: { id: item.id }
                  }"
               ...
               <!-- view -->
               <b-link
                  :to="{ name: 'ViewEmployeeSendTrain', params: { id: item.id } }"
                  v-b-tooltip.hover.top="$t('View')"
                  v-if="
                     $can('EmployeeSendTrainView', 'permissions') || $can('EmployeeSendTrainSignerView', 'permissions')
                  "
                  class="mr-1 cursor-pointer"
               >
               ...
               <!-- delete -->
               <b-link
                  v-if="$can('EmployeeSendTrainDelete', 'permissions') && item.canDelete"
               ...
               <!-- Sign -->
               <b-link
                  v-if="$can('EmployeeSendTrainSign', 'permissions') && item.canSign"
               ...
               <!-- Cancel -->
               <template v-if="$can('EmployeeSendTrainCancel', 'permissions') && item.canCancel">
                  <b-link
               ...
               <!-- document history -->
               <HistoryModalButton :id="item.id" :table-id="item.tableId || 88" />
            </div>
         </template>
         <template #status>
            <b-row>
...
export default {
   ...
   methods: {
      ...
      Refresh() {
         this.isBusy = true;
         if (this.$route.name == 'EmployeeSendTrainForSigner') {
            EmployeeSendTrainService.GetListForSigner(this.filter)
               .then((res) => {
            ...
            EmployeeSendTrainService.GetList(this.filter)
               .then((res) => {
                  this.items = res.data.rows;
                  this.filter.total = res.data.total;
               })
      ...
      Delete(item) {
         this.$swal.fire({
            ...
            preConfirm: () => {
               return EmployeeSendTrainService.Delete(item.id)
                  .then(() => {
