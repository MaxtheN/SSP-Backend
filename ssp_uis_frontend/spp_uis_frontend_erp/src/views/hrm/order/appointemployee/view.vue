...
export default {
   ...
   computed: {
      ...
      IframeSrc() {
         return (
            axios.defaults.baseURL + `hrm/AppointEmployee/DownloadPdf?id2=${this.Data?.id2}&lang=${this.getPdfLang()}`
         );
      }
   },
   methods: {
      ...
      DownloadTemplate1() {
         changeDocument.DownloadTemplate('/hrm/AppointEmployee/DownloadTemplate', this.$t('appointemployee'));
      },
      UploadFile(event) {
         changeDocument.UploadFile('/hrm/AppointEmployee/UploadFile', event).then((res) => {
            this.Data.files.length = 0;
            this.Data.files.push(res.data[0]);
            changeDocument
               .Update('/hrm/AppointEmployee/Update', this.Data)
               .then((res) => {
