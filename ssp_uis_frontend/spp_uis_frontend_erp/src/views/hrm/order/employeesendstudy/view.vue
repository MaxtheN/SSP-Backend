...
export default {
   ...
   computed: {
      ...
      IframeSrc() {
         return (
            axios.defaults.baseURL + `hrm/EmployeeSendStudy/DownloadPdf?id2=${this.Data?.id2}&lang=${this.getPdfLang()}`
         );
      }
   },
   methods: {
      Refresh() {
         ...
         EmployeeSendStudyService.Get(this.$route.params.id)
            .then((res) => {
               this.Data = res.data;
            })
            .catch((error) => {
      ...
      DownloadTemplate1() {
         changeDocument.DownloadTemplate('/hrm/EmployeeSendStudy/DownloadTemplate', this.$t('employeesendstudy'));
      },
      UploadFile(event) {
         changeDocument.UploadFile('/hrm/EmployeeSendStudy/UploadFile', event).then((res) => {
            this.Data.files.length = 0;
            this.Data.files.push(res.data[0]);
            changeDocument
               .Update('/hrm/EmployeeSendStudy/Update', this.Data)
               .then((res) => {
