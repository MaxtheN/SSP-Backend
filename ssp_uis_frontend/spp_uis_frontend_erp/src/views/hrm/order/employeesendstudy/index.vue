      ...
      <form-table-hrm
         ...
         :actions="{
            create: {
               ...
               permission: 'EmployeeSendStudyCreate'
            },
            edit: {
               name: 'EditEmployeeSendStudy',
               permission: 'EmployeeSendStudyEdit'
            },
            delete: {
               name: 'EditEmployeeSendStudy',
               permission: 'EmployeeSendStudyDelete'
            }
         }"
         @request="Refresh"
      >
         <template #cell(actions)="{ item }">
            <div class="text-center" style="text-wrap: nowrap">
               <!-- edit -->
               <b-link
                  v-if="$can('EmployeeSendStudyEdit', 'permissions')"
                  :to="{
                     name: 'EditEmployeeSendStudy',
                     params: { id: item.id }
                  }"
               ...
               <!-- view -->
               <b-link
                  :to="{ name: 'ViewEmployeeSendStudy', params: { id: item.id } }"
                  v-b-tooltip.hover.top="$t('View')"
                  v-if="
                     $can('EmployeeSendStudyView', 'permissions') || $can('EmployeeSendStudySignerView', 'permissions')
                  "
                  class="mr-1 cursor-pointer"
               >
               ...
               <!-- delete -->
               <b-link
                  v-if="$can('EmployeeSendStudyDelete', 'permissions') && item.canDelete"
               ...
               <!-- Sign -->
               <b-link
                  v-if="$can('EmployeeSendStudySign', 'permissions') && item.canSign"
               ...
               <!-- Cancel -->
               <template v-if="$can('EmployeeSendStudyCancel', 'permissions') && item.canCancel">
                  <b-link
               ...
               <!-- document history -->
               <HistoryModalButton :id="item.id" :table-id="item.tableId || 143" />
            </div>
         </template>
         <template #status>
            <b-row>
...
import EmployeeSendStudyService from '@/services/hrm/employeesendstudy.service';
const DefaultFilter = {
   search: '',
   sortBy: '',
   statusId: null,
...
export default {
   ...
   watch: {
      ...
      filter: {
         handler(newValue, oldValue) {
            localStorage.setItem('filterData13', JSON.stringify(newValue));
   ...
   created() {
      if (JSON.parse(localStorage.getItem('filterData13'))) {
         this.filter = JSON.parse(localStorage.getItem('filterData13'));
      } else {
   ...
   methods: {
      ...
      Refresh() {
         this.isBusy = true;
         if (this.$route.name == 'EmployeeSendStudyForSigner') {
            EmployeeSendStudyService.GetListForSigner(this.filter)
               .then((res) => {
            ...
            EmployeeSendStudyService.GetList(this.filter)
               .then((res) => {
                  this.items = res.data.rows;
                  this.filter.total = res.data.total;
               })
      ...
      Delete(item) {
         this.$swal.fire({
            ...
            preConfirm: () => {
               return EmployeeSendStudyService.Delete(item.id)
                  .then(() => {
      ...
      Cancel(item) {
         this.$swal.fire({
            ...
            preConfirm: (msg) => {
               return EmployeeSendStudyService.Cancel({
                  id: item.id,
                  message: msg,
                  signedData: 'cancel'
               })
