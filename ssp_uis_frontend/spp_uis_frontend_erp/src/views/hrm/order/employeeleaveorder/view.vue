   ...
   computed: {
      ...
      IframeSrc() {
         return (
            ...
            `hrm/EmployeeLeaveOrder/DownloadPdf?id2=${this.Data?.id2}&lang=${this.getPdfLang()}`
         );
      }
   },
   methods: {
      ...
      DownloadTemplate1() {
         changeDocument.DownloadTemplate('/hrm/EmployeeLeaveOrder/DownloadTemplate', this.$t('employeeleaveorder'));
      },
      UploadFile(event) {
         changeDocument.UploadFile('/hrm/EmployeeLeaveOrder/UploadFile', event).then((res) => {
            this.Data.files.length = 0;
            this.Data.files.push(res.data[0]);
            changeDocument
               .Update('/hrm/EmployeeLeaveOrder/Update', this.Data)
               .then((res) => {
