...
export default {
   ...
   computed: {
      ...
      IframeSrc() {
         return axios.defaults.baseURL + `hrm/TempCalcKind/DownloadPdf?id2=${this.Data?.id2}&lang=${this.getPdfLang()}`;
      }
   },
   methods: {
      Refresh() {
      ...
      DownloadTemplate1() {
         changeDocument.DownloadTemplate('/hrm/TempCalcKind/DownloadTemplate', this.$t('tempcalckind'));
      },
      UploadFile(event) {
         changeDocument.UploadFile('/hrm/TempCalcKind/UploadFile', event).then((res) => {
            this.Data.files.length = 0;
            this.Data.files.push(res.data[0]);
            changeDocument
               .Update('/hrm/Chastisement/Update', this.Data)
               .then((res) => {
