   ...
   <b-overlay :show="show">
      <b-card>
         <validation-observer ref="ValidationDTO">
            <b-row>
               <b-col sm="12" md="3">
                  <form-input-hrm v-model="Data.code" :label="$t('orderCode')" :placeholder="$t('orderCode')" />
               </b-col>
               <b-col sm="12" md="3">
...
import PartisanshipService from '@/services/hrm/partisanship.service';
// components
...
export default {
   ...
   created() {
      ...
      this.show = true;
      PartisanshipService.Get(this.$route.params.id)
         .then((res) => {
            this.Data = res.data;
         })
         .catch((error) => {
      ...
      SaveData() {
         this.$refs.ValidationDTO.validate().then((success) => {
            if (success) {
               ...
               PartisanshipService.Update(this.Data)
                  .then(() => {
                     this.makeToast(this.$t('SaveSuccess'), 'success');
                     this.$router.push({ name: 'Partisanship' });
                  })
                  .catch((err) => {
