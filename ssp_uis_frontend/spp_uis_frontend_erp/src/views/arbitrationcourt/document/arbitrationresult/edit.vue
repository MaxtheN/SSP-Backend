                  ...
                  <b-row class="align-content-center">
                     ...
                     <b-col sm="12" md="3">
                        <form-input-hrm
                           ...
                           required-star
                           rules="required"
                           v-model="Data.arbitrationCourtApplication.arbitrationAmount"
                     ...
                     <b-col sm="12" md="3">
                        <form-input-hrm
                           required-star
                           rules="required"
                           v-model="Data.payedAmount"
                           :label="$t('payedAmount')"
                           :placeholder="$t('payedAmount')"
                        />
                     </b-col>
                     ...
                     <b-col sm="12" md="3">
                        ...
                        <b-badge class="mt-1" variant="info" v-if="Data.files.length && Data.files[0].id"
                           ...
                           <b-link style="margin-left: 5px; margin-right: 4px" @click="DownloadFile">
                              ...
                              <feather-icon size="18" style="color: black" v-if="!DownloadLoading" icon="DownloadIcon">
                              </feather-icon>
                           </b-link>
                           <b-link @click="DeleteFile">
                              <b-spinner v-if="DeleteLoading" small></b-spinner>
                              <feather-icon size="18" style="color: black" v-if="!DeleteLoading" icon="Trash2Icon">
                              </feather-icon> </b-link
                        ></b-badge>
                     </b-col>
                     <b-col sm="12" md="3" class="mt-2">
...
import ArbitrationResultService from '@/services/arbitrationcourt/arbitrationresult.service.js';
export default {
   components: {
      BOverlay,
      BInputGroupAppend,
   ...
   created() {
      if (this.$route.query.courtId) {
         ArbitrationResultService.GetByArbitrationCourtApplicationId(this.$route.query.courtId)
            .then((res) => {
               this.Data = res.data;
               this.Data.canByDivided = this.$route.query.canByDivided;
            })
            .catch((error) => {
         ...
         ArbitrationResultService.Get(this.$route.params.id)
            .then((res) => {
               this.Data = res.data;
               if (res.data.signs.length != 0) {
                  this.arbitrationJudgeIds = res.data.signs?.map((item) => item.arbitrationJudgeId);
               }
            })
            .catch((error) => {
               this.makeToast(error.response.data.errors, 'danger');
   ...
   methods: {
      ...
      Sign(key, isPinfl) {
         ...
         ArbitrationResultService.Sign(this.filter)
            .then(() => {
      ...
      UploadFile(event) {
         ...
         ArbitrationResultService.UploadFile(formData).then((res) => {
      ...
      DeleteFile() {
         ...
         ArbitrationResultService.DeleteFile(this.Data.files[0].id).then((_res) => {
            this.DownloadLoading = false;
            this.Data.file.splice(0, 1);
         });
      },
      Print(file) {
      ...
      DownloadFile() {
         ArbitrationResultService.DownloadFile(this.Data.files[0].id).then((res) => {
      ...
      SaveData() {
         this.$refs.ValidationDTO.validate().then((success) => {
            if (success) {
               ...
               this.Data.signs = [];
               this.arbitrationJudgeIds.forEach((item, index) => this.Data.signs.push({ arbitrationJudgeId: item }));
               ArbitrationResultService.Update(this.Data)
                  .then((res) => {
                     this.makeToast(this.$t('SaveSuccess'), 'success');
                     ArbitrationResultService.Get(res.data.id)
                        .then((res1) => {
                           this.Data = res1.data;
                           if (res1.data.signs.length != 0) {
                              this.arbitrationJudgeIds = res1.data.signs?.map((item) => item.arbitrationJudgeId);
                           }
                        })
                        .catch((error) => {
                           this.makeToast(error.response.data.errors, 'danger');
