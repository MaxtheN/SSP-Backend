      ...
      <b-card>
         <validation-observer ref="ValidationDTO">
            <b-row>
               ...
               <b-col sm="12" md="3">
                  <form-input-hrm v-model="Data.code" :label="$t('Code')" :placeholder="$t('Code')" />
               </b-col>
               <b-col sm="12" md="3">
...
import EducationItemService from '@/services/hrm/educationitem.service';
// components
...
export default {
   ...
   created() {
      ...
      this.show = true;
      EducationItemService.Get(this.$route.params.id)
         .then((res) => {
            this.Data = res.data;
         })
         .catch((error) => {
      ...
      SaveData() {
         this.$refs.ValidationDTO.validate().then((success) => {
            if (success) {
               ...
               EducationItemService.Update(this.Data)
                  .then(() => {
                     this.makeToast(this.$t('SaveSuccess'), 'success');
                     this.$router.push({ name: 'EducationItem' });
                  })
                  .catch((err) => {
