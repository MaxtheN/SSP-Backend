               ...
               <b-thead>
                  ...
                  <b-tr>
                     <b-th rowspan="2" style="font-weight: 900; font-size: 14px; color: black"
                        >{{ $t('Ажратилган кредит суммаси ') }}<br />
                        <span style="color: red; white-space: nowrap"> {{ $t('( млрд. сўмгача)') }}</span></b-th
                     >
                     <b-th colspan="3" style="font-weight: 900; font-size: 14px; color: black">{{
                        $t('ажратиладиган кредитлар учун кафиллик миқдори')
                     }}</b-th>
                  </b-tr>
                  <b-tr>
                     <b-th>{{ $t('count') }}</b-th>
                     ...
                     <b-th
                        >{{ $t('51-100 нафар') }} <br />
                        <span style="color: red; white-space: nowrap"> {{ $t('(3 млрд. сўмгача)') }}</span>
                     </b-th>
                     <b-th
                        >{{ $t('101-200 нафар') }} <br />
                        <span style="color: red; white-space: nowrap">{{ $t('(4 млрд. сўмгача)') }}</span>
                     </b-th>
                     <b-th
...
export default {
   ...
   methods: {
      ...
      Print() {
         ReportService.SaveAsExcelFund(this.filter).then((res) => {
            this.forceFileDownload(res, this.$t('jamgarma'));
         });
      },
      Refresh() {
         this.isBusy = true;
