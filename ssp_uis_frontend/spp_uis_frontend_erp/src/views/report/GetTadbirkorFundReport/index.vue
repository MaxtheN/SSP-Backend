<template>
             ...
             <b-table-simple hover small caption-top responsive border>
                <b-thead>
                   ...
                   <b-tr>
                      ...
                      <b-th colspan="3" style="font-weight:900;font-size:14px;color:black">{{ $t('мол-мулк ва ер солиғи') }}</b-th>
                      <b-th colspan="2" style="font-weight:900;font-size:14px;color:black">{{ $t('ортиқча тўланган солиқ суммасини қайтариш') }}</b-th>
                      <b-th colspan="2" style="font-weight:900;font-size:14px;color:black">{{ $t('ҚҚСнинг салбий фарқ суммасини қайтарилиши') }}</b-th>
                      <b-th rowspan="2"><span style="font-weight:900;font-size:14px;color:black">{{ $t('Ижтимоий солиқ') }}</span> <br/> {{ $t('(меҳнатга ҳалқ тўлашнинг ЭКМнинг 1 баробаридан кам бўлмаган миқдорда ҳисобланганда солиқ ставкасининг 50 фоизида тўлайда)') }}</b-th>
                      <b-th rowspan="2"><span style="font-weight:900;font-size:14px;color:black">{{ $t('Даромад солиғи') }}</span> <br/> {{ $t('(меҳнатга ҳалқ тўлашнинг ЭКМнинг 1 баробаридан кам бўлмаган миқдорда ҳисобланганда ойлик иш ҳақи бўйича тўланган даромад солиғи қайтарилади))') }}</b-th>
                   </b-tr>
                   <b-tr>
                      ...
                      <b-th>{{ $t('count') }}</b-th>
                      <b-th>{{ $t('workerCount') }}</b-th>
                      <b-th>{{ $t('51-100 нафар') }} <br/> <span style="color:red;white-space:nowrap"> {{ $t('(50 фоиз)') }}</span> </b-th>
                      <b-th>{{ $t('101-200 нафар') }} <br/> <span style="color:red;white-space:nowrap">{{ $t('(75 фоиз)') }}</span>  </b-th>
                      <b-th>{{ $t('200 нафардан ортиқ') }} <br/><span style="color:red;white-space:nowrap">{{ $t('(100 фоиз)') }}</span>  </b-th>
                      <b-th>{{ $t('51-200 нафар') }} <br/> <span style="color:red;white-space:nowrap">{{ $t('(5 кунда)') }}</span> </b-th>
                      <b-th>{{ $t('200 нафардан ортиқ') }} <br/><span style="color:red;white-space:nowrap">{{ $t('(1 кунда)') }}</span>  </b-th>
                      <b-th>{{ $t('51-200 нафар') }} <br/><span style="color:red;white-space:nowrap">{{ $t('(3 - 15 кунда)') }}</span>  </b-th>
                      <b-th>{{ $t('200 нафардан ортиқ') }} <br/><span style="color:red;white-space:nowrap">{{ $t('((1 кунда))') }}</span>  </b-th>
                   </b-tr>
                </b-thead>
                <b-tbody v-if="items.rows.length > 0">
                   <b-tr v-for="(item, idx) in items.rows" :key="idx + 'abc'">
                      ...
                      <b-td class="text-right">{{ item.countCertificates['1'].item1 + item.countCertificates['2'].item1 }}</b-td>
                      <b-td class="text-right">{{ item.countCertificates['3'].item1 }}</b-td>
                      <b-td class="text-right">{{ item.countCertificates['1'].item1 + item.countCertificates['2'].item1 }}</b-td>
                      <b-td class="text-right">{{ item.countCertificates['3'].item1 }}</b-td>
                      <b-td class="text-right">{{ item.totalCertificate.item1 }}</b-td>
                ...
                <b-tfoot v-if="items.rows.length > 0">
                   <b-tr variant="secondary">
                      ...
                      <b-td class="text-right">{{ items.certificateColumnTotals['1'].item1 + items.certificateColumnTotals['2'].item1  }}</b-td>
                      <b-td class="text-right">{{ items.certificateColumnTotals['3'].item1 }}</b-td>
                      <b-td class="text-right">{{ items.certificateColumnTotals['1'].item1 + items.certificateColumnTotals['2'].item1  }}</b-td>
                      <b-td class="text-right">{{ items.certificateColumnTotals['3'].item1 }}</b-td>
                      <b-td class="text-right">{{ items.certificateTotals.item1 }}</b-td>
 ...
 export default {
    ...
    methods: {
       ...
       Refresh() {
          this.isBusy = true;
          ReportService.GetTadbirkorFundReport(this.filter)
             .then((res) => {
                this.items = res.data;
             })
             .catch((error) => {
