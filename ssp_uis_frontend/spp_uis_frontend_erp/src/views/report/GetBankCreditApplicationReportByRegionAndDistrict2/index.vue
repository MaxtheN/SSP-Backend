   ...
   <b-card no-body>
      <div class="m-2">
         <b-row>
            ...
            <b-col sm="12" md="2">
               <b-button @click="Refresh" :disabled="isBusy" variant="primary" class="mt-2 ml-1">
            ...
            <b-table-simple class="report-table" hover caption-top responsive striped border>
               <b-thead>
                  ...
                  <b-tr>
                     ...
                     <b-th>{{ $t('Рад этилган аризалар') }}</b-th>
               ...
               <b-tbody v-if="items.length > 0">
                  <b-tr v-for="(item, idx) in items" :key="idx + 'abc' + 1">
                     ...
                     <b-td class="text-right">
                        {{ currency(item.submittedCount) }}
                     </b-td>
                     <b-td class="text-right">
                        {{ currency(item.submittedSum) }}
                     </b-td>
                     <b-td class="text-right">
                        {{ currency(item.canceledCount + item.rejectedCount) }}
                     </b-td>
               ...
               <b-tfoot v-if="items.length > 0">
                  <b-tr variant="secondary">
                     ...
                     <!-- Application -->
                     <b-td class="text-right">
                        {{ currency(totals.submittedCount) }}
                     </b-td>
                     <b-td class="text-right">
                        {{ currency(totals.submittedSum) }}
                     </b-td>
                     <b-td class="text-right">
                        {{ currency(totals.rejectAllCancel) }}
                     </b-td>
         ...
         ReportService.GetBankCreditApplicationReportByRegionAndDistrict(this.filter)
            .then((res) => {
               ...
               res.data.forEach((item) => {
                  this.totals.submittedCount += item.submittedCount;
                  this.totals.submittedSum += item.submittedSum;
                  this.totals.rejectAllCancel += item.canceledCount + item.rejectedCount;
                  this.totals.employeeCount = item.newVacanciesCount;
