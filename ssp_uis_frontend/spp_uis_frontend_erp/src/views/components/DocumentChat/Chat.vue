<template>
   <div style="position: fixed; bottom: 2rem; top: auto; right: 1.8rem; z-index: 1200; display: inline-block">
      <div class="d-flex flex-column card shadow-lg overflow-hidden" v-if="historySidebar">
         <div
            class="chat-conversation-header p-1 bg-white d-flex justify-content-between align-items-center"
            style="flex: none"
         >
            ...
            <b-button variant="outline" size="sm" @click="historySidebar = !historySidebar">
               <feather-icon icon="XIcon" scale="1.5" />
            </b-button>
         </div>
         <div class="chat-conversation p-1">
            ...
            <ul v-else class="list-unstyled" id="chat-container">
               <li v-for="item in chatMessagesData" :key="item.id" :class="{ right: item.userId }">
         ...
         <div class="p-1 chat-input-section bg-white position-sticky" style="bottom: 0">
            <form @submit.prevent="formSubmit" class="row">
               ...
               <div class="col-auto">
                  <button
                     type="submit"
                     :disabled="!messageText || sendLoading"
      ...
      <button
         ...
         class="pmd-floating-action-btn btn bg-light"
      >
         <feather-icon icon="MessageCircleIcon" class="text-success" size="30" />
      </button>
   </div>
</template>
...
import DocumentChatService from '@/services/others/documentchat.service';
