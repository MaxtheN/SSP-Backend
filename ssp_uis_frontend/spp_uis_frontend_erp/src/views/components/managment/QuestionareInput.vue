<template>
   <div>
      <b-input-group>
         <b-form-input
            :value="value"
            @keyup.enter="() => !hidePlus && $emit('add')"
            @input="(e) => $emit('input', e)"
            v-bind="$attrs"
         ></b-form-input>
         <b-input-group-append>
            <b-button v-b-tooltip.hover :title="$t('Translates')" variant="primary" @click="OpenTranslateModal">
               <feather-icon icon="GlobeIcon"></feather-icon>
            </b-button>
            <b-button v-b-tooltip.hover :title="$t('Save')" variant="success" v-if="!hidePlus" @click="$emit('add')">
               <feather-icon icon="CheckIcon"></feather-icon>
            </b-button>
            <b-button v-b-tooltip.hover :title="$t('Delete')" variant="danger" v-if="!hideX" @click="$emit('close')">
   ...
   methods: {
      ...
      ChangeTranslate() {
         this.TranslateItem.language = !!this.TranslateItem.languageId
            ? this.languageList.filter((item) => item.value == this.TranslateItem.languageId)[0].text
            : '';
      },
      AddTranslate() {
         ...
         if (
            this.TranslateItems.filter((item) => {
               return (
                  item.languageId === this.TranslateItem.languageId && item.columnName == this.TranslateItem.columnName
               );
            }).length > 0
         ) {
      ...
      handleSave() {
         ...
         this.$emit('update:translates', [
            ...this.translates.filter((e) => e.columnName != this.columnName),
            ...this.TranslateItems
         ]);
      }
   }
