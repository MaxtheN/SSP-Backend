   ...
   <form-table-hrm
      ...
      :actions="{
         create: {
            ...
            permission: 'CallCenterAppealCreate'
         },
         edit: {
            name: 'EditCallCenterAppeal',
            permission: 'CallCenterAppealEdit'
         },
         delete: {
            name: 'EditCallCenterAppeal',
            permission: 'CallCenterAppealDelete'
         }
      }"
      @request="Refresh"
   >
      ...
      <template #filter>
         <b-row>
            ...
            <b-col cols="12" md="3" class="d-flex align-items-center justify-content-between">
               ...
               <!-- <template v-if="$can('CallCenterAppealCreate', 'permissions')"> -->
               <b-button variant="primary" :to="{ name: 'EditCallCenterAppeal', params: { id: 0 } }">
      ...
      <template #cell(actions)="{ item }">
         <div class="text-center" style="text-wrap: nowrap">
            <!-- document history -->
            <HistoryModalButton class="mr-1" :id="item.id" :table-id="item.tableId || 145" />
            <b-link
               v-if="$can('CallCenterAppealView', 'permissions')"
               :to="{
                  name: 'EditCallCenterAppeal',
                  params: { id: item.id },
            ...
            <!-- edit -->
            <b-link
               v-if="$can('CallCenterAppealEdit', 'permissions') && item.canEdit"
               :to="{ name: 'EditCallCenterAppeal', params: { id: item.id }, query: { inn: item.contractorInn } }"
               v-b-tooltip.hover.top="$t('Edit')"
               class="mr-1 cursor-pointer"
            >
            ...
            <!-- delete -->
            <b-link
               v-if="$can('CallCenterAppealDelete', 'permissions') && item.canDelete"
            ...
            <!-- accept -->
            <b-link
               v-if="$can('CallCenterAppealAccept', 'permissions') && item.canAccept"
               @click="Accept(item)"
            ...
            <!-- Reject -->
            <template v-if="$can('CallCenterAppealReject', 'permissions') && item.canReject">
               <b-link
...
export default {
   ...
   methods: {
      Print() {
         ...
         CallCenterAppealService.PrinCallCenterAppealExcel(this.filter).then((res) => {
      ...
      Delete(item) {
         this.$swal.fire({
            ...
            preConfirm: () => {
               return CallCenterAppealService.Delete(item.id)
                  .then(() => {
      ...
      Accept(item) {
         this.$swal.fire({
            ...
            showLoaderOnConfirm: true,
            preConfirm: () => {
               return CallCenterAppealService.Accept({
                  id: item.id,
                  message: ''
               })
                  .then(() => {
      ...
      Reject(item) {
         this.$swal.fire({
            ...
            showLoaderOnConfirm: true,
            preConfirm: () => {
               return CallCenterAppealService.Reject({
                  id: item.id,
                  message: ''
               })
                  .then(() => {
      ...
      Refresh() {
         this.isBusy = true;
         CallCenterAppealService.GetList(this.filter)
            .then((res) => {
