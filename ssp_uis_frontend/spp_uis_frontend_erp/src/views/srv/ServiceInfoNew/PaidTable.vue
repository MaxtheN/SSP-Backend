   ...
   <b-table-simple hover small caption-top responsive border>
      <b-thead>
         <b-tr>
            ...
            <b-th rowspan="2">{{ $t('services') }}</b-th>
         ...
         <b-tr>
            ...
            <template v-for="index in items">
               <b-th :key="index">{{ $t('count') }}</b-th>
               <b-th :key="index">{{ $t('Суммаси') }}</b-th>
            </template>
         </b-tr>
         <b-tr>
            <b-th colspan="2">{{ $t('Total') }}</b-th>
            <b-th style="text-wrap: nowrap">{{ currency(totalCounts.totalCount) }}</b-th>
            <b-th style="text-wrap: nowrap">{{ currency(totalCounts.totalSum) }}</b-th>

            <template v-for="(region, index) in items">
               <b-th style="text-wrap: nowrap" :key="index">{{ currency(region.count) }}</b-th>
               <b-th style="text-wrap: nowrap" :key="index">{{ currency(region.sum) }}</b-th>
            </template>
         </b-tr>
      </b-thead>
      <b-tbody>
         ...
         <template v-for="(group, groupIndex) in collectGroupedServices()">
            ...
            <template v-for="(service, serviceIndex) in group.services">
               <b-tr :key="serviceIndex">
                  ...
                  <b-td class="text-right">{{ currency(service.totalSum) }}</b-td>
                  ...
                  <template v-for="(region, regionIndex) in items">
                     ...
                     <b-td :key="regionIndex" class="text-right">{{
                        currency(getRegionData(service, filter.byDistrict ? region.districtId : region.regionId).sum)
                     }}</b-td>
                  </template>
               </b-tr>
            </template>
...
<script>
import { BTableSimple, BThead, BTbody, BTr, BTd, BTh, VBTooltip } from 'bootstrap-vue';
export default {
   name: 'PaidTable',
   props: {
      items: {
         ...
         return this.items.reduce(
            (totals, item) => {
               return {
                  ...
                  totalSum: totals.totalSum + (item.sum || 0)
               };
            },
   ...
   methods: {
      collectGroupedServices() {
         ...
         this.items.forEach((region) => {
            region?.deedGroupCollection?.deedGroup?.forEach((group) => {
               ...
               group.serviceNames.forEach((service) => {
                  ...
                  serviceKey.totalSum += service.sum || 0;
