      ...
      <b-row class="justify-content-end">
         <b-col sm="12" md="8" lg="8">
            <b-card>
               <b-tabs class="nav-tabs nav-justified">
                  ...
                  <b-tab :title="$t('ServiceDeed')" v-if="$route.params.id > 0">
                     <b-overlay
                        :show="!iframeLoaded"
                        v-if="Contract.id2"
         ...
         <b-col sm="6" md="3" lg="3">
            <div>
               ...
               <b-alert class="mt-1" v-if="Contract.message" show variant="danger"
                  ><p class="p-1">{{ Contract.message }}</p>
               </b-alert>
            </div>
         </b-col>
      </b-row>
...
export default {
   ...
   computed: {
      IframeSrcContract() {
         return (
            axios.defaults.baseURL + `srv/ServiceDeed/DownloadPdf?id2=${this.Contract?.id2}&lang=${this.getPdfLang()}`
         );
      }
   },
   created() {
      if (this.$route.query.srvContractId && this.$route.params.id == 0) {
         this.GetDeedByContract();
      } else {
         this.GetDeed();
      }
   },
   methods: {
      ...
      GetDeedByContract() {
         this.show = true;
         ServiceDeedService.GetBySrvContractId(this.$route.query.srvContractId)
            .then((res) => {
               this.Contract = res.data;
               this.Contract.id = 0;
               ...
               ServiceDeedService.Get()
                  .then((res1) => {
                     this.$set(this.Contract, 'docNumber', res1.data.docNumber);
                     this.$set(this.Contract, 'docOn', res1.data.docOn);
                  })
                  .catch((error) => {
                     this.showApiError(error);
                  })
      ...
      Reject() {
         this.$swal.fire({
            ...
            preConfirm: (msg) => {
               return ServiceDeedService.Reject({
                  id: this.Contract.id,
                  message: msg
               })
                  .then(() => {
      ...
      SaveData() {
         ...
         ServiceDeedService.Update(this.Contract)
            .then((res) => {
               this.makeToast(this.$t('SaveSuccess'), 'success');
               if (this.Contract.id == 0) {
                  this.$router.push({ name: 'EditServiceDeed', params: { id: res.data.id } });
               }
               this.GetDeed();
            })
            .catch((err) => {
