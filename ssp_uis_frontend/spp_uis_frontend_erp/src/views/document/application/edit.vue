      ...
      <b-row>
         <b-col sm="12" md="12" lg="12" class="text-center">
            <b-row :class="!Application.prtnContractId && 'justify-content-center'">
               <b-card :style="{ width: isMobileDevice() ? '80%' : '50%' }">
                  <iframe
                     style="width: 100%; height: 100vh"
                     v-if="Application.id2"
                     :src="
                        axios.defaults.baseURL +
                        `Application/PrintApplicationPdf?Id=${Application.id2}&lang=${getPdfLang()}`
                     "
                     frameborder="0"
                  ></iframe>
                  <div class="d-flex">
               ...
               <b-card style="margin-left: 5px" v-if="Data.id2" :style="{ width: isMobileDevice() ? '80%' : '49%' }">
                  <div style="height: 100vh; overflow-y: scroll" v-html="HtmlDataContract" class="pl-2 pr-3"></div>
                  <div class="d-flex">
                     <b-button
                        v-if="Application.canAccept"
                        class="mt-2 mr-2"
      ...
      GetContract() {
         PrtnContractService.GetByApplicationId(this.$route.params.id)
            .then((res) => {
               this.show = false;
               this.Data = res.data;
               PrtnContractService.GetHtmlTemplate(res.data)
                  .then((res1) => {
