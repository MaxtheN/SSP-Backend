            ...
            <b-card>
               <validation-observer ref="ValidationDTO">
                  <b-row>
                     ...
                     <b-col sm="12" md="4" class="mb-1">
                        <form-input v-model="Data.factor" required :label="$t('factor')" />
                     </b-col>
                     <b-col sm="12" md="4" class="mb-1">
                        <form-input v-model="Data.normativeDoc" required :label="$t('normativedoc')" />
                     </b-col>
                     <b-col sm="12" md="4">
                        <form-select
...
export default {
   ...
   created() {
      this.show = true;
      TaxBenefitTypeService.Get(this.$route.params.id)
         .then((res) => {
            this.Data = res.data;
         })
         .catch((error) => {
      ...
      SaveData() {
         this.$refs.ValidationDTO.validate().then((success) => {
            if (success) {
               ...
               TaxBenefitTypeService.Update(this.Data)
                  .then((res) => {
