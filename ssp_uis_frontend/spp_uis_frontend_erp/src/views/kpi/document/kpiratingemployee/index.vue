   ...
   <form-table-hrm
      ...
      :actions="{
         create: {
            ...
            permission: 'KpiRatingEmployeeCreate'
         },
         edit: {
            name: 'EditKpiRatingEmployee',
            permission: 'KpiRatingEmployeeEdit'
         },
         delete: {
            name: 'EditKpiRatingEmployee',
            permission: 'KpiRatingEmployeeDelete'
         }
      }"
      @request="Refresh"
   >
      ...
      <template #filter>
         <b-row>
            <b-col cols="12" md="6" :class="{ 'mb-1': isMobileDevice() }">
               <template>
                  <b-button variant="primary" :to="{ name: 'EditKpiRatingEmployee', params: { id: 0 } }">
         ...
         <b-row align-v="center">
            <!-- <b-col cols="12" md="8">
               <StatusSelect v-model="filter.statusId" @input="Refresh" :filter="[1, 8, 37, 38, 24]" />
            </b-col> -->
         </b-row>
      </template>
      ...
      <template #cell(actions)="{ item }">
         <div class="text-center" style="text-wrap: nowrap">
            <!-- view -->
            <b-link
               v-if="$can('KpiRatingEmployeeViewAll', 'permissions')"
               :to="{
                  name: 'EditKpiRatingEmployee',
                  params: { id: item.id },
                  query: { isView: true }
            ...
            <!-- edit -->
            <b-link
               v-if="$can('KpiRatingEmployeeEdit', 'permissions') && item.canEdit"
               :to="{ name: 'EditKpiRatingEmployee', params: { id: item.id } }"
               v-b-tooltip.hover.top="$t('Edit')"
               class="mr-1 cursor-pointer"
            >
            ...
            <!-- delete -->
            <b-link
               v-if="$can('KpiRatingEmployeeDelete', 'permissions') && item.canDelete"
            ...
            <!-- accept -->
            <b-link
               v-if="$can('KpiRatingEmployeeAccept', 'permissions') && item.canAccept"
               @click="Accept(item)"
            ...
            <!-- Reject -->
            <template v-if="$can('KpiRatingEmployeeCancel', 'permissions') && item.canCancel">
               <b-link
   ...
   methods: {
      Delete(item) {
         this.$swal.fire({
            ...
            preConfirm: () => {
               return KpiRatingEmployeeService.Delete(item.id)
                  .then(() => {
      ...
      Accept(item) {
         this.$swal.fire({
            ...
            showLoaderOnConfirm: true,
            preConfirm: () => {
               return KpiRatingEmployeeService.Accept({
                  id: item.id,
                  message: ''
               })
                  .then(() => {
      ...
      Reject(item) {
         this.$swal.fire({
            ...
            showLoaderOnConfirm: true,
            preConfirm: () => {
               return KpiRatingEmployeeService.Cancel({
                  id: item.id,
                  message: ''
               })
                  .then(() => {
      ...
      Refresh() {
         this.isBusy = true;
         KpiRatingEmployeeService.GetList(this.filter)
            .then((res) => {
