   ...
   >
      ...
      <template #filter>
         <b-row>
            <b-col cols="12" md="6">
               <template>
                  <b-button variant="primary" :to="{ name: 'EditKpiGrating', params: { id: 0 } }">
                     <feather-icon icon="PlusIcon"></feather-icon>
                     {{ $t('create') }}
                  </b-button>
               </template>
            </b-col>
            <b-col cols="12" md="6" class="d-flex no-wrap" :class="{ 'mt-1': isMobileDevice() }">
               <b-input-group>
      ...
      <template #cell(actions)="{ item }">
         <div class="text-center" style="text-wrap: nowrap">
            <!-- view -->
            <b-link
               v-if="$can('KpiGratingViewAll', 'permissions')"
               :to="{
                  name: 'EditKpiGrating',
                  params: { id: item.id },
                  query: { isView: true }
            ...
            <!-- edit -->
            <b-link
               v-if="$can('KpiGratingEdit', 'permissions') && item.canEdit"
               :to="{ name: 'EditKpiGrating', params: { id: item.id } }"
               v-b-tooltip.hover.top="$t('Edit')"
               class="mr-1 cursor-pointer"
            >
            ...
            <!-- delete -->
            <b-link
               v-if="$can('KpiGratingDelete', 'permissions') && item.canDelete"
            ...
            <!-- accept -->
            <b-link
               v-if="$can('KpiGratingAccept', 'permissions') && item.canAccept"
               @click="Accept(item)"
            ...
            <!-- Reject -->
            <template v-if="$can('KpiGratingCancel', 'permissions') && item.canCancel">
               <b-link
   ...
   methods: {
      Delete(item) {
         this.$swal.fire({
            ...
            preConfirm: () => {
               return KpiGratingService.Delete(item.id)
                  .then(() => {
      ...
      Accept(item) {
         this.$swal.fire({
            ...
            showLoaderOnConfirm: true,
            preConfirm: () => {
               return KpiGratingService.Accept({
                  id: item.id,
                  message: ''
               })
                  .then(() => {
      ...
      Reject(item) {
         this.$swal.fire({
            ...
            showLoaderOnConfirm: true,
            preConfirm: () => {
               return KpiGratingService.Cancel({
                  id: item.id,
                  message: ''
               })
                  .then(() => {
      ...
      Refresh() {
         this.isBusy = true;
         KpiGratingService.GetList(this.filter)
            .then((res) => {
