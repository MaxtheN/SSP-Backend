               ...
               <validation-observer ref="ValidationDTO">
                  <b-row>
                     <b-col sm="12" md="4" class="mb-1">
                        <form-input v-model.number="Data.orderCode" type="number" :label="$t('orderCode')" />
                     </b-col>
                     <b-col sm="12" md="4" class="mb-1">
                        <form-input v-model.number="Data.code" type="number" required :label="$t('kode')" />
                     </b-col>
                     <b-col sm="12" md="4" class="mb-1">
                        <form-input v-model.number="Data.textCode" required :label="$t('textCode')" />
                     </b-col>
                     <b-col sm="12" md="4" class="mb-1">
                     ...
                     <b-col sm="12" md="4" class="mb-1">
                        <form-select :options="InstituteList" required-star v-model="Data.ownerId" label="Institute"></form-select>
                     </b-col>
...
export default {
   ...
   created() {
      this.show = true;
      SpecialtyService.Get(this.$route.params.id)
         .then((res) => {
            this.Data = res.data;
         })
         .catch((error) => {
      ...
      SaveData() {
         this.$refs.ValidationDTO.validate().then((success) => {
            if (success) {
               ...
               SpecialtyService.Update(this.Data)
                  .then((res) => {
