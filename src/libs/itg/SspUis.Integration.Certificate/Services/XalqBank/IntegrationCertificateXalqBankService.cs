...
{
    public class IntegrationCertificateXalqBankService :
        StatusGenericHandler,
        IIntegrationCertificateXalqBankService
    {
        private readonly HttpClient _httpClient;
        private readonly IntegrationCertificateConfig _config;

        public IntegrationCertificateXalqBankService(IntegrationCertificateConfig config)
        {
            ...
            if (config.UseProxy)
            {
                ...
                _httpClient.DefaultRequestHeaders.Add("Client-Auth", $"Basic {config.BasicTokenXalqBank}");
            }
            else
                _httpClient.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Basic", config.BasicTokenXalqBank);
        }

        public async Task<IntegrationCertificateResponseDto> PostCertificateXalqBank(IntegrationCertificateRequestDto dto)
        {
            try
            {
                var request = new List<object>() { dto};
                var dtoJson = JsonConvert.SerializeObject(request, Formatting.Indented);

                var url = $"{_config.ApiXalqBank}/api/v1/ssp/certificates";
