using SspUis.DataLayer.EfClasses.Kpi;
using System;
using System.Linq;

namespace SspUis.DataLayer.Repositories
{
    public static class KpiRatingEmployeeFilter
    {
        public static IQueryable<KpiRatingEmployee> ByDocNumber(this IQueryable<KpiRatingEmployee> source, string docNumber)
        {
            if (string.IsNullOrEmpty(docNumber))
                throw new ArgumentException($"{nameof(docNumber)} cannot be null or empty string", nameof(docNumber));
            return source.Where(a => a.DocNumber == docNumber);
        }
    }
}
