...
{
    public class ClaimApplicationService : BaseApplicationService
        <ClaimApplication,
        ...
        ClaimApplicationSortFilterOptions>, IClaimApplicationService
    {
        ...
        public ClaimApplicationService(IUnitOfWork unitOfWork,
        ...
        public override PagedResult<ClaimApplicationListDto> GetList(ClaimApplicationSortFilterOptions options)
        {
            ...
            {
                dto.Rows = dto.Rows.Where(x => x.OrganizationId != null
                    && x.OrganizationId == _authService.User.OrganizationId);
            }

            if (options.CalimAppType)
            {
                dto.Rows = dto.Rows.Where(a => a.ClaimApplicationTypeId == ClaimApplicationTypeIdConst.APILATION_CASSATION ||
                                                a.ClaimApplicationTypeId == ClaimApplicationTypeIdConst.CLAIM_REVISION).ToList();
            }
        ...
        {
            var data = _unitOfWork.Context.Set<Application>()
              .Include(a => a.ClaimApplication)
              .Where(a => a.ContractorId == _authService.Contractor.Id
             && a.ApplicationTypeId == ApplicationTypeIdConst.CLAIM
             && a.StatusId == StatusIdConst.ACCEPTED
             && a.CurrentStepId == StepIdConst.APPLICATION_FOR_COURT_CREATE
             && a.ClaimApplication.ClaimApplicationTypeId == ClaimApplicationTypeIdConst.CLAIM_APPLICATION);
        ...
        public override ClaimApplicationDto Get()
        {
            ...
            {
                AddError("Азолик шартнома яратилмаган !", "Message");
                return null;
            }
        ...
        public ClaimApplicationDto Get(Guid id2)
        {
            ...
            if (dto == null)
            {
                AddError("Ariza topilmadi / Заявление не найдено!", "Message");
                return null;
            }
        ...
        public async Task<HaveId<long>> CreateClaimApplication(CreateClaimApplicationDlDto dto)
        {
            ...
            using (var transaction = _unitOfWork.BeginTransaction())
            {
                try
                {
                    ...
                    if (HasErrors) { AddError("Филе юклаш мажбурий !", "Message"); return null; };
            ...
            using (var transaction = _unitOfWork.BeginTransaction())
            {
                var entity = Repository.Update(dto, ent =>
                {
                    if (!StatusIdConst.CanEditStatuses.Contains(ent.Application.StatusId))
                        AddError("Tahrirlash mumkin emas / Невозможно редактировать", "Message");
                });
        ...
        public async Task Send(SendStatusClaimApplicationDto dto)
        {
            ...
            if (doc == null)
            {
                AddError("По вашему запросу запись не найдено" , "Message");
                return;
            }
            else if (doc.Application.ContractorId != _authService.Contractor.Id)
            {
                AddError("Нет доступа", "Message");
                return;
            }
            ...
            try
            {
                Repository.UpdateStatus(dto, ent =>
                {
                    if (!StatusIdConst.CanClaimApplicationApplyStatus(ent.Application.StatusId, dto.StatusId))
                        AddError("Имкони йўқ / Нет доступа", "Message");
                    else
                    {
        ...
        public async ValueTask<byte[]> DownloadPdf(Guid id2, string? langue)
        {
            var lang = langue ?? "uz-latn";

            var languageId = UnitOfWork.Context.Set<Language>()
                .FirstOrDefault(l => l.Code == lang)?.Id ?? 2;
            ...
            if (wordFile == null)
            {
                AddError("Undefined document Type.", "Message");
                return null;
            }
            ...
            if (claimApplication.Organization.RegionId == 0)
                return null;
        ...
        public ClaimApplicationIntegrationForGetDto GetForIntegration(long id)
        {
            ...
            result.claimApplicationIntegrationFileUrl.ClaimApplicationLink = _systemConf.QrImagePrintPath + "/ClaimApplication/DownloadPdf?Id2=" + claimApplication.Id2 + "&lang=uz-cyrl";
            ...
            }).FirstOrDefault(a => a.ClaimApplicationId == claimApplication.Id);

            if (mediationPlan == null) return result;

            else if (mediationPlan != null)
            result.MediationPlanStatus = mediationPlan.MediatoinPlanStatus;
            result.claimApplicationIntegrationStatusId.MediationPlanStatusId = mediationPlan.MediatoinPlanStatusId;
            result.claimApplicationIntegrationFileUrl.MediationPlanLink = _systemConf.QrImagePrintPath + "/MediationPlan/DownloadPdf?id2=" + mediationPlan.Id2 + "&lang=uz-cyrl";
            ...
            if (mediationPlan != null)
            {
                ...
                }).FirstOrDefault(a => a.MediationPlanId == mediationPlan.Id);

                if (mediation == null) return result;

                result.MediationStatus = mediation.MediationStatus;
                result.claimApplicationIntegrationStatusId.MediationStatusId = mediation.MediationStatusId;
                ...
                var applicationForCourt = _unitOfWork.Context.Set<ApplicationForCourt>().Include(a => a.Files).Select(a => new
                {
                    a.MediationId,
                    ApplicationForCourtStatus = a.Status.FullName,
                    ApplicationForCourtStatusId = a.StatusId,
                    a.Id2,
                    ApplicationForCourtFileId = a.Files.OrderByDescending(f => f.Id).FirstOrDefault().Id,
                }).FirstOrDefault(a => a.MediationId == mediation.Id);
                result.ApplicationForCourtStatus = applicationForCourt?.ApplicationForCourtStatus;
                result.claimApplicationIntegrationStatusId.ApplicationForCourtStatusId = applicationForCourt?.ApplicationForCourtStatusId;
                result.claimApplicationIntegrationFileUrl.ApplicationForCourtLink = applicationForCourt?.ApplicationForCourtFileId != null ? _systemConf.QrImagePrintPath + "/api/ExternalClaimApplication/DownloadFileWithQrCode?id=" + applicationForCourt.ApplicationForCourtFileId : null;
            }
            return result;
        }
            ...
            where TDto : ClaimApplicationDlDto<TDto>
        {
            ...
            {
                Repository.AddError("A'zolik shartnomasi mavjud emas / Нет соглашения о членстве", "Message");
            }
                ...
                || dto.ClaimApplicationTypeId == ClaimApplicationTypeIdConst.CLAIM_REVISION)
            {
                if (!dto.PrevApplicationId.HasValue)
                    Repository.AddError("Oldingi murojaatni tanlang / Выберите предыдущее заявление", "Message");
            }
        }
        ...
        public async Task<ClaimApplicationIntegrationResponseDto> CreateFromIntegration(ClaimApplicationIntegrationRequestDto dto)
        {
            ...
            try
            {
                ...
                //{
                //    AddError("Сиз https://my.chamber.uz/ тизимдан рўйхатдан ўтмагансиз / Вы не зарегистрированы в системе https://my.chamber.uz/");
                //    log.IsSuccess = false;
                //    log.ResponseContent = "Сиз https://my.chamber.uz/ тизимдан рўйхатдан ўтмагансиз / Вы не зарегистрированы в системе https://my.chamber.uz/";
                ...
                {
                    AddError("(827) A'zolik shartnomasi mavjud emas / Нет соглашения о членстве" , "Message");
                    log.IsSuccess = false;
                ...
                {
                    AddError($"(837) Siz da'volik arizasi yaratib bo'lgansiz {dto.ContractIdentificationNumber} / Вы создали претензию {dto.ContractIdentificationNumber}", "Message");
                    log.IsSuccess = false;
                ...
                if (dto.Document.ClaimApplicationTypeId == ClaimApplicationTypeIdConst.APILATION_CASSATION && dto.Document.PrevApplicationId == null)
                {
                    AddError($"Avvalgi muroja'at raqami kiritilmagan / Не введён номер предыдущего обращения", "Message");
                    log.IsSuccess = false;
                    log.ResponseContent = "Avvalgi muroja'at raqami kiritilmagan / Не введён номер предыдущего обращения";
                    log.ResponseStatus = 400;
                    return null;
                }
                ...
                // banklar user qo'shilgandan keyin buni commentdan olish kere
                ...
                var storedFileIds = new List<StorageFileInfo> { };
                Random rand = new Random();
                if (dto.Document.Files == null || dto.Document.Files.Any(a => a.FileAsBase64.IsNullOrEmpty()))
                {
                    AddError($"Fayl biriktirilmagan / Файл не прикреплен", "Message");
                    log.IsSuccess = false;
                    log.ResponseContent = "Fayl biriktirilmagan / Файл не прикреплен";
                    log.ResponseStatus = 400;
                    return null;
                }
                foreach (var file in dto.Document.Files)
                {
                    var fileId = SaveFile($"{_integrationAuthService.User.Inn}_{rand.Next(10000)}", file.FileAsBase64, file.FileExtension);
                    if (HasErrors)
                    {
                        AddError($"(866) Message: File Formati To'g'ri Emas To'g'irlab qaytadan o'rinib kuring ", "Message");
                        log.IsSuccess = false;
                        log.ResponseContent = " Message: File Formati To'g'ri Emas To'g'irlab qaytadan o'rinib kuring";
                        log.ResponseStatus = 400;
                ...
                var createDto = new CreateClaimApplicationDlDto(contractor)
                {
                    ...
                    PrevApplicationId = dto.Document.PrevApplicationId,
                    //DocOn = DateTime.Now.AsDateOnly(),
                    //ContractorPositionName = "Директор",
                    ...
                    BankBranchName = dto.Document.BankBranchName,
                    BankResponsiblePerson = dto.Document.BankResponsiblePerson,
                    ...
                    //Application = _unitOfWork.ApplicationRepository.CrudServices.ReadManyNoTracked<ApplicationDto>().FirstOrDefault(a => a.Id == checkMemshipContract.),
                    //DocNumber = "test",
                    Files = storedFileIds.Select(file => new ClaimApplicationFileDlDto
                    {
                        Id = file.FileId,
                ...
                if (dto.Document.Tables == null || !dto.Document.Tables.Any())
                {
                    AddError($"Javobgarlar maʼlumoti to‘ldirilmagan / Жавобгарлар маълумоти тўлдирилмаган / Информация об респонденте не заполнена", "Message");
                    log.IsSuccess = false;
                    log.ResponseContent = "Javobgarlar maʼlumoti to‘ldirilmagan / Жавобгарлар маълумоти тўлдирилмаган / Информация об респонденте не заполнена";
                    log.ResponseStatus = 400;
                    return null;
                }
                ...
                var application = Create(createDto);

                _unitOfWork.Save();
                if (HasErrors)
                {
                    AddError($"(943) Тизимда хатолик юз берди / File Save qilishda hatolik", "Message");
                    log.IsSuccess = false;
                    log.ResponseContent = " File Save qilishda hatolik";
                    log.ResponseStatus = 400;
                    log.RequestAt = DateTime.Now;
                    return null;
                }
                var entity = _unitOfWork.Context.Set<Application>()
                    .Find(application.Id);

                _storageService.MoveToPersistent(DocumentStorageConst.DOC_CLAIM_APPLICATION_FILES, entity.ClaimApplication.Id.ToString(), storedFileIds.Select(x => x.FileId).ToArray());
                //var application = _unitOfWork.Context.Set<Application>().FirstOrDefault(a => a.Id == entity.Id);
                if (application != null)
                {
            ...
            {
                AddError($"(960) Тизимда хатолик юз берди /MoveToPersistent.. and another", "Message");
                log.IsSuccess = false;
        ...
        private StorageFileInfo SaveFile(string documentId, string data, string fileExtension)
        {
            try
            {
                MemoryStream ms;
                ...
                {
                    ms = new(memoryStream.ToArray());
                }

                StorageFileInfo fileInfo = (StorageFileInfo)_storageService.SaveTemp(
                    DocumentStorageConst.DOC_CLAIM_APPLICATION_FILES,
                    //documentId.ToString(),
                    new StorageFile(Guid.NewGuid(), $"{documentId}{fileExtension}", ms)
                    ).FirstOrDefault()!;
                ...
                if (HasErrors)
                {
                    AddError($"Message: File Formati Tog'ri Emas..", "Message");
                    return null!;
                }
        ...
        private string GetFileDataFromMinio(string documentId, Guid fileId)
        {
            ...
            {
                AddError($"Message: {ex.Message}, StackTrace: {ex.StackTrace}", "Message");
                return null!;
            }
