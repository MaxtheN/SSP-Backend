...
{
    public class BankListDtoConfig : PerDtoConfig<BankListDto, Bank>
    {
        public override Action<IMappingExpression<Bank, BankListDto>> AlterReadMapping =>
            cfg => cfg
                .ForMember(x => x.BankName, x => x.MapFrom(ent => ent.Translates.AsQueryable()
                    .FirstOrDefault(BankTranslate.GetExpr(BankTranslateColumn.bank_name, ServiceProvider.CultureHelper.CurrentCulture.Id)).TranslateText ?? ent.BankName))
