...
public class RecallLeaveService
    : BaseEntityService<long, RecallLeave, RecallLeaveListDto, RecallLeaveDto, CreateRecallLeaveDlDto, UpdateRecallLeaveDlDto, IRecallLeaveRepository, RecallLeaveSortFilterOptions>
    , IRecallLeaveService
{
    ...
    private readonly IRecallLeaveRepository _repository;
    private readonly INumberService _numberService;
    private readonly IConvertService _pdfConverter;

    public RecallLeaveService(IUnitOfWork unitOfWork,
        ...
        IDocumentChangeLogService documentChangeLogService, IConvertService pdfConverter) : base(unitOfWork)
    {
        ...
        this._repository = unitOfWork.RecallLeaveRepository;
        ...
        _linkConfig = linkConfigs.FirstOrDefault(config => config.Name == "RecallLeave");
    }
    public PagedResult<RecallLeaveListDto> GetList(RecallLeaveSortFilterOptions options)
    {
        return Repository.ReadAsNoTracked<RecallLeaveListDto>()
            .SortFilter(options)
            .AsPagedResult(options);
    }
    public PagedResult<RecallLeaveListDto> GetListForSigner(RecallLeaveSortFilterOptions dto)
    {
        var result = _repository.ReadAsNoTracked<RecallLeaveListDto>().Where(a => new int[]
        {
    ...
    public SelectList<long> AsSelectList(RecallLeaveSortFilterOptions options)
    {
        return Repository.ReadAsNoTracked<RecallLeaveListDto>()
           .SortFilter(options)
           .AsSelectList();
    }
    public RecallLeaveDto Get()
    {
        return new RecallLeaveDto
        {
            DocOn = DateOnly.FromDateTime(DateTime.Now),
            DocNumber = _numberService.GetNext(NumberTemplateDocumentConst.DOC_RECALL_LEAVE, 1).Item2
        };
    }

    public override RecallLeaveDto Get(long id)
    {
        var dto = _repository.ById<RecallLeaveDto>(id);
        CombineStatuses(_repository);
        if (IsValid)
        {
            var nextSigner = _unitOfWork.Context.Set<RecallLeaveSigner>()
                    .Where(a => a.OwnerId == dto.Id && !a.SignedAt.HasValue)
                    .OrderBy(a => a.SignOrder).FirstOrDefault();

            dto.CanSign = StatusIdConst.CanApplyEmployeeLeaveOrder(dto.StatusId, StatusIdConst.MODIFIED) && _authService.HasPermission(ModuleCode.RecallLeaveSign) && (nextSigner == null || (nextSigner != null && nextSigner.EmployeeManageId == _authService.User.EmployeeManageId));
            dto.CanModify = StatusIdConst.CanApplyEmployeeLeaveOrder(dto.StatusId, StatusIdConst.MODIFIED) && _authService.HasPermission(ModuleCode.RecallLeaveEdit);
            dto.CanCancel = StatusIdConst.CanApplyEmployeeLeaveOrder(dto.StatusId, StatusIdConst.NOT_ACCEPTED) && _authService.HasPermission(ModuleCode.RecallLeaveCancel);
            dto.CanDelete = StatusIdConst.CanApplyEmployeeLeaveOrder(dto.StatusId, StatusIdConst.DELETED) && _authService.HasPermission(ModuleCode.RecallLeaveDelete);
        }
        return dto;
    }
    public async ValueTask<HaveId<long>> Create(CreateRecallLeaveDlDto dto)
    {
        using(var transaction = UnitOfWork.BeginTransaction())
        {
            try
            {
                ...
                UnitOfWork.Save();
                var res = CreateDocumentChangeLog(entity.Id, entity.StatusId, "CreateRecallLeave");
                CombineStatuses(Repository);
                if(IsValid)
                {
                    transaction.Commit();

                    //var model = _unitOfWork.Context.Set<RecallLeave>()
                    //                     .Include(a => a.Signer)
    ...
    public async ValueTask<string> PostToIMZOAndSentUrl(RecallLeave contract)
    {
    ...
    public async ValueTask<string?> SendUrl(long contractId)
    {
        ...
        var contract = _unitOfWork.Context.Set<RecallLeave>()
                                                          .Include(a => a.Signer)
                                                              .ThenInclude(a => a.EmployeeManage)
                                                                  .ThenInclude(em => em.Employee)
                                                                      .ThenInclude(e => e.Organization)
    ...
    public WbImzoCreateSignRequestDto CreatDtoForRequestSign(RecallLeave contract)
    {
        ...
        return new WbImzoCreateSignRequestDto
        {
            ...
            IsForceCreate = false,
            TableId = TableIdConst.DOC_RECALL_LEAVE,
    ...
    public WebImzoDto ConvertToDto(RecallLeave dto)
    => new()
    {
        DocOn = dto.DocOn,
    ...
    public override void Update(UpdateRecallLeaveDlDto dto)
    {
        using(var transaction = UnitOfWork.BeginTransaction())
        {
            try
            {
                ...
                UnitOfWork.Save();
                var res = CreateDocumentChangeLog(entity.Id, entity.StatusId, "UpdateRecallLeave");
                CombineStatuses(Repository);
                if(IsValid)
                    transaction.Commit();
            }
    ...
    public void Accept(UpdateStatusRecallLeaveDto dTo, bool isLoged = false)
    {
        try
        {
            ...
            UpdateStatusRecallLeaveDlDto dto = new UpdateStatusRecallLeaveDlDto { Id = dTo.Id, StatusId = StatusIdConst.ACCEPTED };
    ...
    public async Task Sign(SignStatusRecallLeaveDto dto)
    {
        using (var transaction = _unitOfWork.BeginTransaction())
        {
            try
            {
                ...
                if (IsValid)
                {
                    ...
                    {
                        Accept(new UpdateStatusRecallLeaveDto
                        {
                            Id = dto.Id,
    ...
    private Guid SaveFile(long docId, string data, string fileName)
    {
        ...
        writer.Flush();
        ms.Position = 0;
        var fileInfo = _storageService.Save($"{nameof(TableIdConst.DOC_RECALL_LEAVE)}_SIGN_DATA", docId.ToString(), new StorageFile(Guid.NewGuid(), fileName, ms));

        return fileInfo.FirstOrDefault().FileId;
    }
    public void Cancel(UpdateStatusRecallLeaveDto dTo)
    {
        var dto = new UpdateStatusRecallLeaveDlDto { Id = dTo.Id, StatusId = StatusIdConst.NOT_ACCEPTED };
        UpdateStatus(dto, ent =>
        {
    ...
    public override void Delete(long id)
    {
        using(var transaction = UnitOfWork.BeginTransaction())
        {
            try
            {
                ...
                var statusDto = new UpdateStatusRecallLeaveDlDto()
                {
                    Id = id,
                ...
                UnitOfWork.Save();
                var res = CreateDocumentChangeLog(entity.Id, StatusIdConst.DELETED, "DeleteRecallLeave");
    ...
    public async Task<byte[]> DownloadPdf(Guid id2, string? lang)
    {
        ...
        var wordFile = _storageService.GetStaticFile(
           StaticFileConst.WordTemplate.GetFileName(
               language,
               StaticFileConst.WordTemplate.RE_CALL_LEAVE)
           );

        var dto = Repository.ReadAsNoTracked<RecallLeaveDto>(applyFilter: false)
        ...
        for (int i = 1; i < dto.Signer.Count; i++)
        {
            ...
            {
                dto.Signer[i].QrSign.Text = "\n Lavozimi:" + dto.Signer[i].Position;
            }
    ...
    private HaveId<long> UpdateStatus(UpdateStatusRecallLeaveDlDto dto, Action<RecallLeave> validation)
    {
        using (var transaction = UnitOfWork.BeginTransaction())
        {
    ...
    {
        var moveDto = Repository.ById<RecallLeaveDto>(id, applyFilter: false);
    ...
    private void Validation<TDto>(RecallLeaveDlDto<TDto> dto, RecallLeave entity)
    ...
    public byte[]? GetWordTemplate()
    {
        ...
        var wordFile = _storageService.GetStaticFile(
            StaticFileConst.WordTemplate.GetFileName(lang, StaticFileConst.WordTemplate.RE_CALL_LEAVE));
    ...
    public object UploadFiles(params StorageFile[] files)
    {
        ...
        var result = _storageService.SaveTemp(DocumentStorageConst.DOC_RE_CALL_LEAVE, files)
            .Select(a => new ReCallLeaveFileDto
            {
                Id = a.FileId,
                FileName = a.FileName,
