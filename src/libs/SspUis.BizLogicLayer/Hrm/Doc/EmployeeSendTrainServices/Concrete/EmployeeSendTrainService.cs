...
public partial class EmployeeSendTrainService
    : BaseEntityService<long, EmployeeSendTrain, EmployeeSendTrainListDto, EmployeeSendTrainDto, CreateEmployeeSendTrainDlDto, UpdateEmployeeSendTrainDlDto, IEmployeeSendTrainRepository, EmployeeSendTrainSortFilterOptions>
    , IEmployeeSendTrainService
{
    private readonly IAuthService _authService;
    private readonly IDocumentChangeLogService _documentChangeLogService;
    private readonly IEmployeeSendTrainRepository _repository;
    ...
    public EmployeeSendTrainService(
        ...
        : base(unitOfWork)
    {
        this._repository = unitOfWork.EmployeeSendTrainRepository;
        ...
        _linkConfig = linkConfigs.FirstOrDefault(config => config.Name == "EmployeeSendTrain");
    }
    public PagedResult<EmployeeSendTrainListDto> GetList(EmployeeSendTrainSortFilterOptions options)
    {
        return Repository.ReadAsNoTracked<EmployeeSendTrainListDto>()
            .SortFilter(options)
            .AsPagedResult(options);
    }

    public PagedResult<EmployeeSendTrainListDto> GetListForSigner(EmployeeSendTrainSortFilterOptions dto)
    {
        var result = _repository.ReadAsNoTracked<EmployeeSendTrainListDto>(q =>
    ...
    //public PagedResult<UnpaidEmployeeSendTrainListDto> GetUnPaidList(UnpaidEmployeeSendTrainSortFilterPageOptions dto)
    //{
    ...
    //                        a.DocumentSysTableId == TableIdConst.DOC_EMPLOYEE_SEND_TRAIN)
    //            .Select(a => a.DocumentTableId);

    //    var result = _repository.CrudServices.ProjectFromEntityToDto<EmployeeSendTrainTable, UnpaidEmployeeSendTrainListDto>(a => a.Where(b => true))
    ...
    public EmployeeSendTrainDto Get()
    {
        return new EmployeeSendTrainDto
        {
            DocOn = DateOnly.FromDateTime(DateTime.Now),
            DocNumber = _numberService.GetNext(NumberTemplateDocumentConst.DOC_EMPLOYEE_SEND_TRAIN, 1).Item2
        };
    }

    public EmployeeSendTrainDto Get(long id)
    {
        var dto = _repository.ById<EmployeeSendTrainDto>(id);
        CombineStatuses(_repository);
        if (IsValid)
        {
            var nextSigner = _unitOfWork.Context.Set<EmployeeSendTrainSigner>()
                   .Where(a => a.OwnerId == dto.Id && !a.SignedAt.HasValue)
                   .OrderBy(a => a.SignOrder).FirstOrDefault();

            dto.CanSign = StatusIdConst.CanApplyEmployeeLeaveOrder(dto.StatusId, StatusIdConst.MODIFIED) && _authService.HasPermission(ModuleCode.EmployeeSendTrainSign) && (nextSigner == null || (nextSigner != null && nextSigner.EmployeeManageId == _authService.User.EmployeeManageId));
            dto.CanModify = StatusIdConst.CanApplyEmployeeLeaveOrder(dto.StatusId, StatusIdConst.MODIFIED) && _authService.HasPermission(ModuleCode.EmployeeSendTrainEdit);
            dto.CanAccept = StatusIdConst.CanApplyEmployeeLeaveOrder(dto.StatusId, StatusIdConst.ACCEPTED) && _authService.HasPermission(ModuleCode.EmployeeSendTrainAccept);
            dto.CanCancel = StatusIdConst.CanApplyEmployeeLeaveOrder(dto.StatusId, StatusIdConst.NOT_ACCEPTED) && _authService.HasPermission(ModuleCode.EmployeeSendTrainCancel);
            dto.CanDelete = StatusIdConst.CanApplyEmployeeLeaveOrder(dto.StatusId, StatusIdConst.DELETED) && _authService.HasPermission(ModuleCode.EmployeeSendTrainDelete);
        }
        return dto;
    }
    
    public async ValueTask<HaveId<long>> Create(CreateEmployeeSendTrainDlDto dto)
    {
        using(var transaction = UnitOfWork.BeginTransaction())
        {
            try
            {
                ...
                UnitOfWork.Save();
                var res = CreateDocumentChangeLog(entity.Id, entity.StatusId, "CreateEmployeeSendTrain");
                CombineStatuses(Repository);
                if (IsValid)
                {
                    transaction.Commit();

                    //var model = _unitOfWork.Context.Set<EmployeeSendTrain>()
                    //                     .Include(a => a.Signer)
    ...
    public async ValueTask<string> PostToIMZOAndSentUrl(EmployeeSendTrain contract)
    {
    ...
    public async ValueTask<string?> SendUrl(long contractId)
    {
        ...
        var contract = _unitOfWork.Context.Set<EmployeeSendTrain>()
                                                          .Include(a => a.Signer)
                                                              .ThenInclude(a => a.EmployeeManage)
                                                                  .ThenInclude(em => em.Employee)
                                                                      .ThenInclude(e => e.Organization)
    ...
    public WbImzoCreateSignRequestDto CreatDtoForRequestSign(EmployeeSendTrain contract)
    {
        ...
        return new WbImzoCreateSignRequestDto
        {
            ...
            IsForceCreate = false,
            TableId = TableIdConst.DOC_EMPLOYEE_SEND_TRAIN,
    ...
    public WebImzoDto ConvertToDto(EmployeeSendTrain dto)
    => new()
    {
        DocOn = dto.DocOn,
    ...
    public override void Update(UpdateEmployeeSendTrainDlDto dto)
    {
        using(var transaction = UnitOfWork.BeginTransaction())
        {
            try
            {
                ...
                UnitOfWork.Save();
                var res = CreateDocumentChangeLog(entity.Id, entity.StatusId, "UpdateEmployeeSendTrain");
                CombineStatuses(Repository);
                if(IsValid)
                    transaction.Commit();
            }
    ...
    public void Accept(UpdateStatusEmployeeSendTrainDto dTo, bool isLoged = false)
    {
        try
        {
            ...
            var dto = new UpdateStatusEmployeeSendTrainDlDto { Id = dTo.Id, StatusId = StatusIdConst.ACCEPTED };
    ...
    public async Task Sign(SignStatusEmployeeSendTrainDto dto)
    {
        using (var transaction = _unitOfWork.BeginTransaction())
        {
            try
            {
                ...
                if (IsValid)
                {
                    ...
                    {
                        Accept(new UpdateStatusEmployeeSendTrainDto
                        {
                            Id = dto.Id,
    ...
    public Guid SaveFile(long docId, string data, string fileName)
    {
        ...
        writer.Flush();
        ms.Position = 0;
        var fileInfo = _storageService.Save($"{nameof(TableIdConst.DOC_EMPLOYEE_SEND_TRAIN)}_SIGN_DATA", docId.ToString(), new StorageFile(Guid.NewGuid(), fileName, ms));

        return fileInfo.FirstOrDefault().FileId;
    }
    public void Cancel(UpdateStatusEmployeeSendTrainDto dTo)
    {
        var dto = new UpdateStatusEmployeeSendTrainDlDto { Id = dTo.Id, StatusId = StatusIdConst.NOT_ACCEPTED };
        UpdateStatus(dto, ent =>
        {
            if (!StatusIdConst.CanApplyEmployeeLeaveOrder(ent.StatusId, dto.StatusId))
                _repository.AddError("Нет доступа");
    ...
    public async Task<byte[]> DownloadPdf(Guid id2)
    {
        ...
        var dto = Repository.ReadAsNoTracked<EmployeeSendTrainDto>(applyFilter: false)
    ...
    public override void Delete(long id)
    {
        using(var transaction = UnitOfWork.BeginTransaction())
        {
            try
            {
                ...
                var statusDto = new UpdateStatusEmployeeSendTrainDlDto()
                {
                    Id = id,
                ...
                UnitOfWork.Save();
                var res = CreateDocumentChangeLog(entity.Id, StatusIdConst.DELETED, "DeleteEmployeeSendTrain");
    ...
    private HaveId<long> UpdateStatus(UpdateStatusEmployeeSendTrainDlDto dto, Action<EmployeeSendTrain> validation)
    {
        using (var transaction = _unitOfWork.BeginTransaction())
        {
            try
            {
                ...
                if (HasErrors)
                    return null;
                _unitOfWork.Save();
                var res = CreateDocumentChangeLog(entity.Id, entity.StatusId, "EmployeeSendTrain");
                if (IsValid)
                    transaction.Commit();
                return res;
            }
    ...
    {
        var moveDto = Repository.ById<EmployeeSendTrainDto>(id, applyFilter: false);
    ...
    private void Validation<TDto>(EmployeeSendTrainDlDto<TDto> dto, EmployeeSendTrain entity)
