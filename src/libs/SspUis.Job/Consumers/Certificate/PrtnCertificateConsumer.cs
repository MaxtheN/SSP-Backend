...
{
    public class PrtnCertificateConsumerConfig : IConsumerConfig<PrtnCertificateMessage>
    {
        public string Name => "PrtnCertificateSync";
        public RabbitQueue Queue => Queues.Partner.PrtnCertificate;
    ...
    public class PrtnCertificateConsumer : StatusGenericHandler, IConsumer<PrtnCertificateMessage>
    {
        ...
        public PrtnCertificateConsumer(IPrtnCertificateService prtnCertificateService,
            IDocumentJobHistoryService documentJobHistoryService,
            IAuthService authService,
            ILogger<PrtnCertificateConsumer> logger,
        ...
        public async Task ConsumeAsync(PrtnCertificateMessage message, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                //statusini ozgartirmay turamiz
                // update doc status as EXECUTING and clear messasge
                //_dbContext.Database.ExecuteSqlRaw($"UPDATE {table.DbSchemaName}.{table.DbTableName} SET STATUS_ID={StatusIdConst.SENDING}, PREV_STATUS_ID = STATUS_ID, MESSAGE='' WHERE ID={message.DocId}");

                await _prtnCertificateService.SentForOtherServiceFormedCertificate(message.DocId, EnumTypeOrganization.Soliq);
            ...
            catch (Exception ex)
            {
                ...
                using (var scope = _serviceProvider.CreateScope())
                {
                    ...
                    using (var command = dbContext.Database.GetDbConnection().CreateCommand())
                    {
                        string sql = $"UPDATE {table.DbSchemaName}.{table.DbTableName} SET STATUS_ID=PREV_STATUS_ID, PREV_STATUS_ID=NULL, MESSAGE='doni' WHERE ID={message.DocId}";
